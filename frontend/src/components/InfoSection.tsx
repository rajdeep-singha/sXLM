        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 mb-12 sm:mb-16 items-start">
          ...
          <div>
            <p className="text-black/70 text-xl sm:text-2xl md:text-3xl leading-relaxed">
              sXLM is a vault share token representing a proportional claim on the XLM
              the vault holds.
            </p>
          </div>
        </div>
...
No hidden<br />dilution
            </p>
            <p className="text-black/70 text-base max-w-xs">
              The exchange rate is computed from the vault balance on every read. It can only
              rise when XLM has actually been contributed , never by an admin setting a number.
            </p>
          </div>
          ...
          <div className="rounded-2xl min-h-80 flex flex-col justify-between p-7" style={{ background: '#2B2644' }}>
            ...
            <p className="text-white/60 text-base">
              Exchange rate history and vault balances are readable on chain. APY reflects
              realized rate movement, not a projection. There is no strategy allocation yet, so
              it is currently flat.
            </p>
          </div>
        </div>
      </div>

...
export class EventListenerService {
    ...
    async processEvent(event) {
        ...
        if (topicName === "deposit" && event.contractId === config.contracts.stakingContractId) {
            ...
            console.log(`[EventListener] Deposit: wallet=${wallet} xlm=${xlmAmount} sxlm=${sxlmMinted} ledger=${event.ledger}`);
            ...
            console.log(`[EventListener] Instant withdrawal: wallet=${wallet} xlm=${xlmAmount} ledger=${event.ledger}`);
            ...
            console.log(`[EventListener] Delayed withdrawal: wallet=${wallet} xlm=${xlmAmount} id=${withdrawalId} unlock=${unlockLedger}`);
            ...
            console.log(`[EventListener] Claim: wallet=${wallet} xlm=${xlmAmount} ledger=${event.ledger}`);
            ...
            console.log(`[EventListener] Rewards added: ${amount} stroops at ledger ${event.ledger}`);
                ...
                try {
                    await this.prisma.liquidationEvent.create({
                        data: {
                            ...
                            debtRepaid: BigInt(values[2]),
                            collateralSeized: BigInt(values[3]),
                            ledger: event.ledger,
                        },
                    });
                }

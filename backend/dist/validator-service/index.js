...
export class ValidatorService {
    ...
    async initialize() {
        ...
        // Run initial fetch
        await this.fetchAndUpdateValidators().catch((err) => console.error("[ValidatorService] Initial fetch failed:", err));
    ...
    async fetchAndUpdateValidators() {
        ...
        for (const validator of validators) {
            ...
            if (metrics.uptime < config.protocol.validatorMinUptime) {
                ...
                console.warn(`[ValidatorService] Validator ${metrics.pubkey} uptime below threshold: ${(metrics.uptime * 100).toFixed(2)}%`);
            }
        }
        console.log(`[ValidatorService] Updated ${validators.length} validators`);
    }
    async fetchValidatorsFromHorizon() {
        try {
    ...
    async fetchKnownValidators() {
        try {
            // Fetch from Stellar Expert or known validator list
            const response = await fetch(`${this.horizonUrl}/ledgers?order=desc&limit=1`);
            if (!response.ok)
                return [];
            ...
            const historyResponse = await fetch(`${this.horizonUrl}/ledgers/${latestLedger.sequence}/operations?limit=10`);
    ...
    async calculateMetrics(validator) {
        ...
        try {
            const accountResponse = await fetch(`${this.horizonUrl}/accounts/${validator.pubkey}`);
            if (accountResponse.ok) {
                const accountData = (await accountResponse.json());
                // Calculate uptime from account activity
                const nativeBalance = accountData.balances.find((b) => b.asset_type === "native");
                if (nativeBalance) {
                    allocatedStake = BigInt(Math.floor(parseFloat(nativeBalance.balance) * 1e7));
                }
                // Check recent transaction history for uptime estimation
                const txResponse = await fetch(`${this.horizonUrl}/accounts/${validator.pubkey}/transactions?order=desc&limit=10`);
                if (txResponse.ok) {
            ...
            console.warn(`[ValidatorService] Error fetching metrics for ${validator.pubkey}:`, err);
        ...
        const performanceScore = this.computePerformanceScore(uptime, commission, votingPower);
        return {
    ...
    computePerformanceScore(uptime, commission, votingPower) {
        ...
        const score = PERFORMANCE_WEIGHTS.uptime * uptimeScore +
    ...
    async getValidatorByPubkey(pubkey) {

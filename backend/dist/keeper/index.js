 ...
 */
import { rpc, Contract, scValToNative, Keypair, TransactionBuilder, BASE_FEE, } from "@stellar/stellar-sdk";
import { config } from "../config/index.js";
import { callAddRewards, callWithdrawFees, callCollectProtocolFees, getLpAccruedProtocolFees, getTreasuryBalance, } from "../staking-engine/contractClient.js";
...
let recalibrateInterval = null;
let treasuryRecycleInterval = null;
export class KeeperBot {
    ...
    async initialize() {
        ...
        await this.runHarvestCycle().catch((err) => console.error("[KeeperBot] Initial harvest cycle failed:", err));
        await this.bumpAllContractTTLs().catch((err) => console.error("[KeeperBot] Initial TTL bump failed:", err));
        ...
        console.log(`[KeeperBot] Running — harvest every ${KEEPER_INTERVAL_MS / 3_600_000}h, TTL bump every ${TTL_BUMP_INTERVAL_MS / 3_600_000}h`);
    }
    async shutdown() {
        if (keeperInterval) {
    ...
    async runHarvestCycle() {
        ...
        if (pendingInterest > BigInt(0)) {
            console.log(`[KeeperBot] Pending interest: ${Number(pendingInterest) / 1e7} XLM`);
        ...
        try {
            ...
            console.log(`[KeeperBot] add_rewards called with ${Number(totalYield) / 1e7} XLM (lending: ${Number(harvested) / 1e7}, LP fees: ${Number(lpProtocolFees) / 1e7}) — sXLM rate will increase`);
        }
        catch (err) {
            console.error("[KeeperBot] add_rewards failed:", err);
            console.error(`[KeeperBot] MANUAL ACTION REQUIRED: call add_rewards with ${totalYield} stroops`);
        }
    }
    // ============================================================
    // Query accrued interest from lending contract
    // ============================================================
    async queryLendingAccruedInterest() {
        try {
            const result = await this.simulateView(config.contracts.lendingContractId, "total_accrued_interest", []);
            return result != null ? BigInt(result) : BigInt(0);
        }
        catch (err) {
    ...
    // ============================================================
    async harvestLendingInterest(pendingBefore) {
        try {
            const hash = await this.executeAdminCall(config.contracts.lendingContractId, "harvest_interest", []);
    ...
    // ============================================================
    async recalibrateStakingRate() {
        try {
            await this.executeAdminCall(config.contracts.stakingContractId, "recalibrate_rate", []);
    ...
    // ============================================================
    async logLpPoolStats() {
        try {
            const reserves = await this.simulateView(config.contracts.lpPoolContractId, "get_reserves", []);
            const arr = reserves;
            const xlm = Number(arr?.[0] ?? 0) / 1e7;
            const sxlm = Number(arr?.[1] ?? 0) / 1e7;
            const k = xlm * sxlm;
            const accruedFees = await getLpAccruedProtocolFees().catch(() => BigInt(0));
            console.log(`[KeeperBot] LP Pool: reserve_xlm=${xlm.toFixed(2)}, reserve_sxlm=${sxlm.toFixed(2)}, k=${k.toFixed(2)}, accrued_protocol_fees=${Number(accruedFees) / 1e7} XLM`);
        }
        catch (err) {
    ...
    // ============================================================
    async recycleTreasury() {
        try {
            const treasuryBal = await getTreasuryBalance();
            if (treasuryBal < TREASURY_RECYCLE_THRESHOLD) {
                console.log(`[KeeperBot] Treasury balance ${Number(treasuryBal) / 1e7} XLM below threshold (${Number(TREASURY_RECYCLE_THRESHOLD) / 1e7} XLM) — skipping recycle`);
                return;
            }
            ...
            console.log(`[KeeperBot] Recycled ${Number(treasuryBal) / 1e7} XLM treasury → add_rewards — stakers get ~100% of yield`);
        }
        catch (err) {
    ...
    // Helpers
    // ============================================================
    async simulateView(contractId, method, args) {
    ...
    async executeAdminCall(contractId, method, args) {
        const keypair = Keypair.fromSecret(config.admin.secretKey);
        const account = await this.server.getAccount(keypair.publicKey());
        const contract = new Contract(contractId);
        const op = contract.call(method, ...args);
    ...
    async pollTransaction(hash, maxAttempts = 30, intervalMs = 2000) {

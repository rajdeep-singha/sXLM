...
export class MetricsCron {
    ...
    async initialize() {
        ...
        // Fetch initial price
        await this.fetchXlmPrice().catch((err) => console.error("[MetricsCron] Initial price fetch failed:", err));
        // Take initial snapshot
        await this.takeMetricsSnapshot().catch((err) => console.error("[MetricsCron] Initial snapshot failed:", err));
    ...
    async takeMetricsSnapshot() {
        ...
        const avgValidatorScore = validators.length > 0
        ...
        console.log(`[MetricsCron] Snapshot: staked=${totalStakedXlm.toFixed(2)} XLM, TVL=$${tvlUsd.toFixed(2)}, avgScore=${avgValidatorScore.toFixed(3)}`);
    }
    getXlmPrice() {

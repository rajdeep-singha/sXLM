 ...
 */
import { rpc, Contract, Address, scValToNative, TransactionBuilder, BASE_FEE, } from "@stellar/stellar-sdk";
...
export class RestakingEngine {
    ...
    async getPosition(wallet) {
        ...
        try {
            ...
            const op = contract.call("get_position", new Address(wallet).toScVal());
            ...
            if (rpc.Api.isSimulationSuccess(simResult) && simResult.result) {
                ...
                const effectiveLeverage = collateral > 0 && borrowed > 0
                    ? collateral / (collateral - borrowed)
                    : 1;
                const hf = borrowed > 0 ? (collateral * 0.7) / borrowed : Infinity;
                const loops = effectiveLeverage > 1 ? Math.round(Math.log(effectiveLeverage) / Math.log(1 / 0.3)) : 0;
                const netAPR = effectiveLeverage * 6 - (effectiveLeverage - 1) * 4; // Using default rates
                return {
                    wallet,
    ...
    async getPositionFromDB(wallet) {

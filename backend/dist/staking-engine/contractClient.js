...
export async function callWithdrawFees(amount) {
...
export async function callCollectProtocolFees() {
    return executeLpPoolAdminCall("collect_protocol_fees");
}
export async function callSetLpProtocolFeeBps(bps) {
    return executeLpPoolAdminCall("set_protocol_fee_bps", [
        nativeToScVal(bps, { type: "u32" }),
    ]);
}
export async function getLpAccruedProtocolFees() {
    const contract = getLpPoolContract();
    const { keypair, account } = await getSourceAccount();
...
async function executeLendingAdminCall(method, args = []) {
    const { keypair, account } = await getSourceAccount();
...
export async function callUpdateCollateralFactor(bps) {
    return executeLendingAdminCall("update_collateral_factor", [
        nativeToScVal(bps, { type: "u64" }),
    ]);
}
export async function callUpdateBorrowRate(bps) {
    return executeLendingAdminCall("update_borrow_rate", [
        nativeToScVal(bps, { type: "u64" }),
    ]);
}
export async function callUpdateLiquidationThreshold(bps) {

...
export function startWithdrawalQueueProcessor(prisma) {
    if (pollInterval) {
        console.warn("[WithdrawalQueue] Processor already running");
        return;
    }
    // Run immediately on start
    processQueue(prisma).catch((err) => console.error("[WithdrawalQueue] Initial processing failed:", err));
    ...
    console.log(`[WithdrawalQueue] Started polling every ${config.protocol.withdrawalPollIntervalMs / 1000}s`);
}
export function stopWithdrawalQueueProcessor() {
    if (pollInterval) {
...
async function processQueue(prisma) {
    ...
    try {
        ...
        console.log(`[WithdrawalQueue] Processing ${readyWithdrawals.length} ready withdrawals`);
        ...
        for (const withdrawal of readyWithdrawals) {
            try {
                ...
                const claimResult = await callClaimWithdrawal(withdrawal.wallet, withdrawal.id);
                // Mark as completed
                await prisma.withdrawal.update({
                    where: { id: withdrawal.id },
                ...
                console.log(`[WithdrawalQueue] Processed withdrawal #${withdrawal.id} for ${withdrawal.wallet}, tx: ${claimResult.txHash}`);
            }
            catch (err) {
                console.error(`[WithdrawalQueue] Failed to process withdrawal #${withdrawal.id}:`, err);
                ...
                const isPermanentFailure = errorMessage.includes("already claimed") ||
...
export async function getQueueStats(prisma) {
...
//# sourceMappingURL=withdrawalQueueProcessor.js.map
...
export class DelegationManager {
     ...
     */
    async calculateRequiredBuffer(totalStakedStroops) {
        ...
        if (withdrawals.length === 0) {
            ...
            const staticBuffer = BigInt(Math.floor(Number(totalStakedStroops) * (config.protocol.liquidityBufferPercent / 100)));
            console.log(`[DelegationManager] No withdrawal history, using static buffer: ${Number(staticBuffer) / 1e7} XLM (${config.protocol.liquidityBufferPercent}%)`);
            return staticBuffer;
        }
        ...
        const staticFloor = BigInt(Math.floor(Number(totalStakedStroops) * (config.protocol.liquidityBufferPercent / 100)));
        const requiredBuffer = demandBuffer > staticFloor ? demandBuffer : staticFloor;
        console.log(`[DelegationManager] Dynamic buffer: D=${(dailyAvgWithdrawal / 1e7).toFixed(2)} XLM/day × α=${safetyFactor} = ${Number(demandBuffer) / 1e7} XLM | Floor: ${Number(staticFloor) / 1e7} XLM | Using: ${Number(requiredBuffer) / 1e7} XLM`);
        return requiredBuffer;
    }
    /**
     * Sync the active validator list to the staking contract on-chain.
     */
    async syncValidatorsToContract() {
        ...
        try {
            ...
            const validatorArgs = pubkeys
                ...
                .join(" ");
            console.log(`[DelegationManager] Synced ${pubkeys.length} validators to contract`);
        }
        catch (err) {
     ...
     */
    async rebalanceDelegations(totalStakedStroops) {
        const validators = await this.prisma.validator.findMany({
            where: {
        ...
        const delegatableAmount = totalStakedStroops > requiredBuffer
            ? totalStakedStroops - requiredBuffer
            : BigInt(0);
        // Calculate target allocations weighted by performance score
        const totalScore = validators.reduce((sum, v) => sum + v.performanceScore, 0);
        const targets = validators.map((v) => {
            const fraction = v.performanceScore / totalScore;
            const targetStake = BigInt(Math.floor(Number(delegatableAmount) * fraction));
        ...
        for (const target of targets) {
            ...
            console.log(`[DelegationManager] ${target.pubkey}: ${direction} by ${Number(absDelta) / 1e7} XLM → ${Number(target.targetStake) / 1e7} XLM`);
        }
        if (changesApplied > 0) {
            await this.syncValidatorsToContract();
        }
        console.log(`[DelegationManager] Delegation rebalance complete: ${changesApplied} changes across ${validators.length} validators (buffer: ${Number(requiredBuffer) / 1e7} XLM)`);
    }
    /**
     ...
     */
    async allocateDeposit(xlmAmountStroops) {
        const validators = await this.prisma.validator.findMany({
            where: {
                uptime: { gte: config.protocol.validatorMinUptime },
            },
        ...
        const delegatable = BigInt(Math.floor(Number(xlmAmountStroops) * (1 - effectiveBufferFraction)));
        ...
        console.log(`[DelegationManager] Allocated ${Number(xlmAmountStroops) / 1e7} XLM deposit across ${validators.length} validators (buffer kept: ${(effectiveBufferFraction * 100).toFixed(1)}%)`);
    }
    /**
     * Deallocate stake from validators when a withdrawal happens.
     * Removes from lowest performers first.
     */
    async deallocateWithdrawal(xlmAmountStroops) {
        const validators = await this.prisma.validator.findMany({
        ...
        console.log(`[DelegationManager] Deallocated ${Number(xlmAmountStroops) / 1e7} XLM from validators`);
    }
    /**
    ...
    async getWeightedProtocolAPR() {
        ...
        if (validators.length === 0)
            return 0;
        const totalStake = validators.reduce((sum, v) => sum + Number(v.allocatedStake), 0);
        if (totalStake === 0)
            return 0;
        const BASE_APR = 0.06;
        let weightedAPR = 0;
    ...
    async getDelegationBreakdown() {
        ...
        const totalStake = validators.reduce((sum, v) => sum + v.allocatedStake, BigInt(0));
        return validators.map((v) => ({
            pubkey: v.pubkey,
            allocatedStake: v.allocatedStake,
            performanceScore: v.performanceScore,
            percentage: totalStake > BigInt(0)
    ...
    async getWithdrawalQueueTime() {
        ...
        const epochLimitStroops = BigInt(Math.floor(Number(await getTotalStaked()) * (config.protocol.liquidityBufferPercent / 100)));

...
export async function refreshRate() {
    ...
    try {
        ...
        console.log(`[ExchangeRateManager] Rate updated: ${rate.toFixed(7)} (1 sXLM = ${rate.toFixed(7)} XLM)`);
        return rate;
    }
    catch (err) {
        ...
        if (cachedRate) {
            console.warn(`[ExchangeRateManager] Using stale rate from ${new Date(cachedRate.fetchedAt).toISOString()}`);
            return cachedRate.rate;
        }
...
export function startPeriodicRefresh() {
    ...
    // Initial fetch
    refreshRate().catch((err) => console.error("[ExchangeRateManager] Initial refresh failed:", err));
    ...
    console.log(`[ExchangeRateManager] Periodic refresh started (every ${config.protocol.exchangeRateRefreshIntervalMs / 1000}s)`);
}
...
export function computeExchangeRate(totalXlmStaked, totalSxlmSupply) {
...
export function xlmToSxlm(xlmStroops, exchangeRate) {
    if (exchangeRate <= 0)
        throw new Error("Invalid exchange rate");
    return BigInt(Math.floor(Number(xlmStroops) / exchangeRate));
}
export function sxlmToXlm(sxlmStroops, exchangeRate) {
    if (exchangeRate <= 0)
        throw new Error("Invalid exchange rate");
    return BigInt(Math.floor(Number(sxlmStroops) * exchangeRate));
}
//# sourceMappingURL=exchangeRateManager.js.map
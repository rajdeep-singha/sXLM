...
let rewardDistributionInterval = null;
...
export class RewardEngine {
    ...
    async initialize() {
        ...
        // Initial snapshot
        await this.takeSnapshot().catch((err) => console.error("[RewardEngine] Initial snapshot failed:", err));
        // Periodic snapshots
        ...
        console.log(`[RewardEngine] Initialized, snapshotting every ${config.protocol.rewardSnapshotIntervalMs / 1000}s (reward distribution handled by KeeperBot)`);
    }
    async shutdown() {
        if (snapshotInterval) {
    ...
    async distributeRewards() {
        ...
        if (validators.length > 0) {
            const totalAllocated = validators.reduce((sum, v) => sum + Number(v.allocatedStake), 0);
            if (totalAllocated > 0) {
                weightedAPR = 0;
        ...
        const rewardAmount = BigInt(Math.floor(compoundedValue - Number(totalStaked)));
        ...
        try {
            ...
            console.log(`[RewardEngine] Distributed ${Number(rewardAmount) / 1e7} XLM in rewards (weighted APR: ${(weightedAPR * 100).toFixed(2)}%)`);
    ...
    async takeSnapshot() {
        ...
        callUpdateLendingExchangeRate(exchangeRate).catch((err) => console.warn("[RewardEngine] Lending rate sync failed (non-fatal):", err));
        ...
        console.log(`[RewardEngine] Snapshot: rate=${exchangeRate.toFixed(7)}, apy=${(apy * 100).toFixed(2)}%`);
    }
    async calculateAPY(currentRate) {
    ...
    async getExchangeRateHistory(days) {
    ...
    async calculatePeriodYield(days) {
        const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);

...
export class RiskEngine {
    ...
    async runHealthCheck() {
        ...
        if (validators.length === 0)
            return;
        const downValidators = validators.filter((v) => v.uptime < config.protocol.validatorMinUptime);
        if (downValidators.length > validators.length * 0.3) {
            if (!this.emergencyMode) {
                ...
                await this.sendGovernanceNotification("EMERGENCY", `${downValidators.length}/${validators.length} validators down. Protocol paused. Emergency rebalance triggered.`);
            }
        }
            ...
            await this.sendGovernanceNotification("RECOVERY", "All validators healthy. Protocol unpaused. Emergency mode deactivated.");
        }
        // Check for individual slashing risk
        for (const validator of downValidators) {
            const hoursSinceCheck = (Date.now() - validator.lastChecked.getTime()) / (1000 * 60 * 60);
            if (hoursSinceCheck > 2 && validator.uptime < 0.9) {
                console.warn(`[RiskEngine] Slashing risk for ${validator.pubkey} — uptime ${(validator.uptime * 100).toFixed(1)}%, stale for ${hoursSinceCheck.toFixed(1)}h`);
                const eventBus = getEventBus();
                await eventBus.publish(EventType.REBALANCE_REQUIRED, {
     ...
     */
    async checkAllocationDeviation(validators) {
        const activeValidators = validators.filter((v) => v.uptime >= config.protocol.validatorMinUptime);
        if (activeValidators.length === 0)
            return;
        const totalScore = activeValidators.reduce((sum, v) => sum + v.performanceScore, 0);
        const totalStake = activeValidators.reduce((sum, v) => sum + v.allocatedStake, BigInt(0));
        if (totalStake === BigInt(0) || totalScore === 0)
            return;
        for (const v of activeValidators) {
            ...
            if (deviation > config.protocol.rebalanceThreshold) {
                console.log(`[RiskEngine] Allocation deviation for ${v.pubkey}: actual=${(actualFraction * 100).toFixed(1)}% target=${(targetFraction * 100).toFixed(1)}%`);
                const eventBus = getEventBus();
                await eventBus.publish(EventType.REBALANCE_REQUIRED, {
     ...
     */
    async executeAutoReallocation(reason) {
        ...
        const healthy = validators.filter((v) => v.uptime >= config.protocol.validatorMinUptime);
        const unhealthy = validators.filter((v) => v.uptime < config.protocol.validatorMinUptime && v.allocatedStake > BigInt(0));
        ...
        const stakeToRedistribute = unhealthy.reduce((sum, v) => sum + v.allocatedStake, BigInt(0));
        if (stakeToRedistribute === BigInt(0))
            return;
        // Distribute proportionally to healthy validators by performance score
        const totalHealthyScore = healthy.reduce((sum, v) => sum + v.performanceScore, 0);
        const plans = [];
        for (const target of healthy) {
            const fraction = target.performanceScore / totalHealthyScore;
            const allocation = BigInt(Math.floor(Number(stakeToRedistribute) * fraction));
            if (allocation > BigInt(0)) {
                ...
                for (const source of unhealthy) {
                    if (source.allocatedStake > BigInt(0)) {
                        const moveAmount = allocation < source.allocatedStake
                            ? allocation
        ...
        for (const plan of plans) {
            console.log(`[RiskEngine] Reallocating ${plan.amount} stroops: ${plan.fromValidator} → ${plan.toValidator}`);
        ...
        console.log(`[RiskEngine] Auto-reallocation complete: ${plans.length} moves executed`);
        await this.sendGovernanceNotification("REBALANCE", `Auto-reallocation executed: ${plans.length} stake moves (reason: ${reason})`);
    }
    async handleValidatorDown(pubkey, uptime) {
        if (uptime < 0.85) {
            console.error(`[RiskEngine] Critical: validator ${pubkey} uptime ${(uptime * 100).toFixed(1)}% — triggering reallocation`);
            ...
            if (validator && validator.allocatedStake > BigInt(0)) {
                ...
                const slashAmount = BigInt(Math.floor(Number(validator.allocatedStake) * slashPercent));
                if (slashAmount > BigInt(0)) {
                    try {
                        await callApplySlashing(slashAmount);
                        console.warn(`[RiskEngine] Applied slashing: ${Number(slashAmount) / 1e7} XLM for validator ${pubkey}`);
                        ...
                        await this.sendGovernanceNotification("SLASHING", `Applied ${(slashPercent * 100).toFixed(0)}% slash (${(Number(slashAmount) / 1e7).toFixed(2)} XLM) for validator ${pubkey} (uptime: ${(uptime * 100).toFixed(1)}%)`);
                    }
                    catch (err) {
     ...
     */
    async sendGovernanceNotification(level, message) {
        const payload = {
            level,
            message,
            protocol: "sXLM",

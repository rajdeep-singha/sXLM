...
async function queryContractView(server, contractId, method, args) {
    const contract = new Contract(contractId);
    const op = contract.call(method, ...args);
...
export const lendingRoutes = async (fastify, opts) => {
    const { prisma } = opts;
    ...
    fastify.post("/lending/deposit-collateral", async (request, reply) => {
        try {
            ...
            const sxlmRaw = await queryContractView(server, config.contracts.sxlmTokenContractId, "balance", [new Address(body.userAddress).toScVal()]);
            ...
            const result = await buildContractTx(server, lendingContractId, "deposit_collateral", [
            ...
            const message = err instanceof Error ? err.message : err?.message ?? "Deposit failed";
    ...
    fastify.post("/lending/withdraw-collateral", async (request, reply) => {
        try {
            const body = amountSchema.parse(request.body);
            const stroops = BigInt(Math.floor(body.amount * 1e7));
            const result = await buildContractTx(server, lendingContractId, "withdraw_collateral", [
                new Address(body.userAddress).toScVal(),
                nativeToScVal(stroops, { type: "i128" }),
            ], body.userAddress);
            return result;
            ...
            const message = err instanceof Error ? err.message : err?.message ?? "Withdraw failed";
            reply.status(400).send({ error: message });
        }
    });
    /**
    ...
    fastify.post("/lending/borrow", async (request, reply) => {
        try {
            ...
            const result = await buildContractTx(server, lendingContractId, "borrow", [
                new Address(body.userAddress).toScVal(),
                nativeToScVal(stroops, { type: "i128" }),
            ], body.userAddress);
            return result;
            ...
            const message = err instanceof Error ? err.message : err?.message ?? "Borrow failed";
            reply.status(400).send({ error: message });
        }
    });
    /**
    ...
    fastify.post("/lending/repay", async (request, reply) => {
        try {
            const body = amountSchema.parse(request.body);
            const stroops = BigInt(Math.floor(body.amount * 1e7));
            const result = await buildContractTx(server, lendingContractId, "repay", [
                new Address(body.userAddress).toScVal(),
                nativeToScVal(stroops, { type: "i128" }),
            ], body.userAddress);
            return result;
            ...
            const message = err instanceof Error ? err.message : err?.message ?? "Repay failed";
            reply.status(400).send({ error: message });
        }
    });
    /**
    ...
    fastify.post("/lending/liquidate", async (request, reply) => {
        try {
            ...
            const result = await buildContractTx(server, lendingContractId, "liquidate", [
            ...
            const message = err instanceof Error ? err.message : err?.message ?? "Liquidation failed";
            reply.status(400).send({ error: message });
        }
    });
    /**
    ...
    fastify.get("/lending/stats", async () => {
        try {
            const [totalCollateral, totalBorrowed, cfBpsRaw, ltBpsRaw, borrowRateBpsRaw, poolBalanceRaw] = await Promise.all([

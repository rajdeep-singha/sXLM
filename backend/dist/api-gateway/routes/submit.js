import { z } from "zod";
import { rpc, TransactionBuilder, Contract, Address, nativeToScVal, scValToNative, BASE_FEE, } from "@stellar/stellar-sdk";
 ...
 */
async function findContractWithdrawal(server, userAddress, maxScan = 50) {
    ...
    for (let id = 0; id < maxScan; id++) {
        try {
            const tx = new TransactionBuilder(adminAccount, {
                ...
                .addOperation(stakingContract.call("get_withdrawal", nativeToScVal(id, { type: "u64" })))
                .setTimeout(30)
                .build();
            const sim = await server.simulateTransaction(tx);
            if (!rpc.Api.isSimulationSuccess(sim) || !sim.result)
                break; // past end of queue
...
export const submitRoutes = async (fastify, opts) => {
    ...
    fastify.post("/staking/submit", async (request, reply) => {
        try {
            ...
            const message = err instanceof Error ? err.message : err?.message ?? "Submit failed";
    ...
    fastify.post("/staking/claim", async (request, reply) => {
        try {
            ...
            const claimOp = contract.call("claim_withdrawal", new Address(body.userAddress).toScVal(), nativeToScVal(withdrawal.id, { type: "u64" }));
            ...
            const message = err instanceof Error ? err.message : err?.message ?? "Claim failed";
            reply.status(400).send({ error: message });
        }
    });
};
 ...
 */
async function pollTransaction(rpcUrl, hash) {

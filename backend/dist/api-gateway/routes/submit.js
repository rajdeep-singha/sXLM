import { z } from "zod";
import { rpc, TransactionBuilder, Contract, Address, nativeToScVal, BASE_FEE, } from "@stellar/stellar-sdk";
...
export const submitRoutes = async (fastify, opts) => {
    ...
    fastify.post("/staking/claim", async (request, reply) => {
        try {
            ...
            const claimOp = contract.call("claim_withdrawal", new Address(body.userAddress).toScVal(), nativeToScVal(Number(body.withdrawalId), { type: "u64" }));
 ...
 */
async function pollTransaction(rpcUrl, hash, maxAttempts = 20, intervalMs = 2000) {

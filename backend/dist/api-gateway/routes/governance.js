...
export const governanceRoutes = async (fastify, opts) => {
    ...
    fastify.post("/governance/create-proposal", async (request, reply) => {
        try {
            ...
            const result = await buildContractTx(server, govContractId, "create_proposal", [
            ...
            await prisma.governanceProposal.create({
                data: {
                    ...
                    status: "active",
                    expiresAt: new Date(Date.now() + votingPeriodLedgers * 5 * 1000 // ~5s per ledger
                    ),
                },
            });
            return result;
    ...
    fastify.post("/governance/vote", async (request, reply) => {
        try {
            ...
            const result = await buildContractTx(server, govContractId, "vote", [
                new Address(body.userAddress).toScVal(),
    ...
    fastify.get("/governance/proposals", async () => {
        try {
            const proposalCount = await queryContractView(server, govContractId, "proposal_count", []);
            const count = Number(proposalCount ?? 0);
            const proposals = [];
            // Fetch each proposal from contract
            for (let i = 0; i < Math.min(count, 50); i++) {
                try {
                    const proposal = await queryContractView(server, govContractId, "get_proposal", [nativeToScVal(BigInt(i), { type: "u64" })]);
                    if (proposal) {
                        const voteCount = await queryContractView(server, govContractId, "get_vote_count", [nativeToScVal(BigInt(i), { type: "u64" })]);
    ...
    fastify.get("/governance/proposals/:id", async (request, reply) => {
        ...
        try {
            const proposal = await queryContractView(server, govContractId, "get_proposal", [nativeToScVal(proposalId, { type: "u64" })]);
            if (!proposal) {
                return reply.status(404).send({ error: "Proposal not found" });
            }
            const voteCount = await queryContractView(server, govContractId, "get_vote_count", [nativeToScVal(proposalId, { type: "u64" })]);
            return {
                id: proposalId,
                proposer: proposal.proposer?.toString() ?? "",
                paramKey: proposal.param_key ?? "",
    ...
    fastify.get("/governance/params", async () => {
        ...
        const params = await Promise.all(paramKeys.map(async ({ key, defaultValue, description }) => {
            ...
            try {
                const onChainValue = await queryContractView(server, govContractId, "get_param", [nativeToScVal(key, { type: "string" })]);

...
export const liquidityRoutes = async (fastify, opts) => {
    const { prisma } = opts;
    ...
    fastify.post("/liquidity/add", async (request, reply) => {
        try {
            ...
            const result = await buildContractTx(server, lpContractId, "add_liquidity", [
    ...
    fastify.post("/liquidity/remove", async (request, reply) => {
        try {
            ...
            const result = await buildContractTx(server, lpContractId, "remove_liquidity", [
                new Address(body.userAddress).toScVal(),
    ...
    fastify.post("/liquidity/swap-xlm-to-sxlm", async (request, reply) => {
        try {
            ...
            const result = await buildContractTx(server, lpContractId, "swap_xlm_to_sxlm", [
                new Address(body.userAddress).toScVal(),
    ...
    fastify.post("/liquidity/swap-sxlm-to-xlm", async (request, reply) => {
        try {
            ...
            const stroops = BigInt(Math.floor(body.amount * 1e7));
            const minOutStroops = BigInt(Math.floor(body.minOut * 1e7));
            const result = await buildContractTx(server, lpContractId, "swap_sxlm_to_xlm", [
                new Address(body.userAddress).toScVal(),
                nativeToScVal(stroops, { type: "i128" }),
                nativeToScVal(minOutStroops, { type: "i128" }),
            ], body.userAddress);
    ...
    fastify.get("/liquidity/position/:wallet", async (request) => {
        ...
        try {
            const lpBalance = await queryContractView(server, lpContractId, "get_lp_balance", [new Address(wallet).toScVal()]);
            ...
            const totalLp = await queryContractView(server, lpContractId, "total_lp_supply", []);
            const totalLpBig = BigInt(totalLp ?? 0);
            const sharePercent = totalLpBig > 0 ? (Number(lpTokens) / Number(totalLpBig)) * 100 : 0;
            const reserveXlm = BigInt(reserves?.[0] ?? 0);
            const reserveSxlm = BigInt(reserves?.[1] ?? 0);
            // Calculate user's share of the pool
            const userXlm = totalLpBig > 0
                ? (lpTokens * reserveXlm) / totalLpBig
                : BigInt(0);
            const userSxlm = totalLpBig > 0
    ...
    fastify.get("/liquidity/pool-stats", async () => {
        try {
            const reserves = await queryContractView(server, lpContractId, "get_reserves", []);
            const price = await queryContractView(server, lpContractId, "get_price", []);
            const totalLp = await queryContractView(server, lpContractId, "total_lp_supply", []);

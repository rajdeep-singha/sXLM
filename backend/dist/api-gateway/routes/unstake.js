import { z } from "zod";
import { rpc, Contract, Address, nativeToScVal, scValToNative, TransactionBuilder, Operation, BASE_FEE, } from "@stellar/stellar-sdk";
...
async function querySxlmBalance(server, userAddress) {
    const tokenContract = new Contract(config.contracts.sxlmTokenContractId);
    const readOp = tokenContract.call("balance", new Address(userAddress).toScVal());
...
async function buildRestoreTx(server, userAddress) {
    const tokenContract = new Contract(config.contracts.sxlmTokenContractId);
    const readOp = tokenContract.call("balance", new Address(userAddress).toScVal());
...
export const unstakeRoutes = async (fastify, opts) => {
    ...
    fastify.post("/staking/unstake", async (request, reply) => {
        try {
            ...
            const message = err instanceof Error ? err.message : err?.message ?? "Unstake failed";
            reply.status(400).send({ error: message });
        }
    });
};
